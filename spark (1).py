...
os.environ["SPARK_HOME"] = "/workspaces/projet-esme/spark-3.2.3-bin-hadoop2.7" # Définition du chemin d'installation de Spark

spark = SparkSession.builder \
    .appName("KafkaWeatherConsumer") \
    .getOrCreate()  # Création de la session Spark

spark.sparkContext.setLogLevel("ERROR") # Réduction du niveau des logs pour éviter le bruit inutile
...
]) # Définition du schéma des données JSON provenant de l'API météo
...
df = (
    spark.readStream
    .format("kafka")
    .option("kafka.bootstrap.servers", "localhost:9092") # Connexion au serveur Kafka
    .option("subscribe", "topic-weather") # Souscription au topic Kafka
    .option("startingOffsets", "earliest") # Définition du point de départ
...
# ---------------------------------------------------------------------------
parsed_df = df.select(
    from_json(col("value").cast(StringType()), weather_schema).alias("data"), #convversion JSON
...
#la temperature ressenti 
processed_df = processed_df.withColumn( #.withColumn() est une méthode utilisée pour ajouter ou modifier une colonne dans un DataFrame Spark
    "heat_index",
    col("temperature") + (
        0.5555 * (
...
#pour savoir la gravité d'une tempête 
...
processed_df = processed_df.withColumn(
    "time_of_day",
    #06h00 - 12h00	= Matin /12h00 - 18h00	= Après-midi /18h00 - 00h00	= Soirée / 00h00 - 06h00= Nuit
    when((col("date").substr(12, 2).cast("int") >= 6) & (col("date").substr(12, 2).cast("int") < 12), "Matin")
    .when((col("date").substr(12, 2).cast("int") >= 12) & (col("date").substr(12, 2).cast("int") < 18), "Après-midi")
    .when((col("date").substr(12, 2).cast("int") >= 18) & (col("date").substr(12, 2).cast("int") < 24), "Soirée")
    .otherwise("Nuit")
)
# ---------------------------------------------------------------------------
...
kafka_output_df = processed_df.select(
    to_json(struct(
        ...
        col("humidity"),
        col("speed"),
        col("heat_index"),       # Ajout de heat_index
        col("severity_index"),   # Ajout de severity_index
        col("time_of_day")       # Ajout de time_of_day
    )).alias("value")  # Convertir les données en JSON pour Kafka
    #Kafka stocke les messages sous forme de clé-valeur
)
